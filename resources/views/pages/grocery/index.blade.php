        ...
        <main class="flex-1 flex flex-col overflow-y-auto p-8 max-w-7xl mx-auto w-full space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <section class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
                    <h2 class="font-bold text-gray-800 text-lg">Batch Import Groceries</h2>
                ...
                <section class="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col" @if(! auth()->check() || ! auth()->user()->hasPermission('upload_file')) style="grid-column: 1 / -1;" @endif>
                    ...
                    <form action="{{ route('grocery.index') }}" method="GET" class="p-4 border-b border-gray-100 bg-gray-50/50 grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                        ...
                        <div class="md:col-span-2 flex gap-1">
                            ...
                            <a href="{{ route('grocery.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 text-xs font-semibold py-2 px-2 rounded-lg transition text-center flex items-center justify-center">✕</a>
                        </div>
                    </form>
                            ...
                            <tbody class="divide-y divide-gray-100 text-sm">
                                @forelse($files as $file)
                                    <tr class="hover:bg-gray-50/70 transition">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="text-gray-900 font-medium">{{ $file->row_count }} records listed</span>
                                        </td>
                                        <td class="px-6 py-4">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-12 text-center text-gray-400 text-sm">
                                            📭 No uploaded grocery documents found.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
