            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <section class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
                    ...
                    <form action="{{ route('payments.upload') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div id="dropzone" class="group border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-rose-500 transition cursor-pointer relative bg-gray-50/50">
                            ...
                            <span class="text-3xl block mb-2">
                                <i id="fileIcon" class="fa-solid fa-file-excel text-gray-300 group-hover:text-rose-500 transition"></i>
                            </span>
                ...
                <section class="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col" @if(! auth()->check() || ! auth()->user()->hasPermission('upload_file')) style="grid-column: 1 / -1;" @endif>
                    ...
                    <form action="{{ route('payments.index') }}" method="GET" class="p-4 border-b border-gray-100 bg-gray-50/50 grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                        <div class="md:col-span-4">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search files..." class="w-full text-xs p-2 border border-gray-300 rounded-lg outline-none bg-white focus:border-rose-500" />
                        </div>
                        <div class="md:col-span-2">
                            <label class="text-xs font-bold uppercase text-gray-400 block mb-1">Status</label>
                            <select name="status" class="w-full text-xs p-2 border border-gray-300 rounded-lg outline-none bg-white focus:border-rose-500">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="date" name="date_from" value="{{ request('date_from') }}" class="w-full text-xs p-1.5 border border-gray-300 rounded-lg outline-none bg-white focus:border-rose-500" />
                        </div>
                        <div class="md:col-span-2">
                            <label class="text-xs font-bold uppercase text-gray-400 block mb-1">To</label>
                            <input type="date" name="date_to" value="{{ request('date_to') }}" class="w-full text-xs p-1.5 border border-gray-300 rounded-lg outline-none bg-white focus:border-rose-500" />
                        </div>
                        <div class="md:col-span-2 flex gap-1">
                            ...
                            <a href="{{ route('payments.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 text-xs font-semibold py-2 px-2 rounded-lg transition text-center flex items-center justify-center">✕</a>
                        </div>
                    </form>
                            ...
                            <tbody class="divide-y divide-gray-100 text-sm">
                                @forelse($files as $file)
                                    <tr class="hover:bg-gray-50/70 transition">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="text-gray-900 font-medium">{{ $file->row_count }} receipts listed</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            ...
                                            @elseif($file->status === 'processing')
                                                <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-rose-50 text-rose-700 border border-rose-200">Processing</span>
                                            @else
